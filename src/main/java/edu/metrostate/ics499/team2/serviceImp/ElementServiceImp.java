...
public class ElementServiceImp implements ElementService {
	...
	@Autowired
	public ElementServiceImp(ElementController elmController) {
...
//		return "";
	...
	@Override
	public boolean isValid(Element item) {
		return elmController.showAll().stream()
		.filter(element -> element.equals(item)).toList().size() > 0 ? false : true;
	}
}